...
- **Error Responses:**
  - **Code:** `400 Bad Request` — `Invalid Body`, `name is required`, `name must be at most 40 characters`, or `<field> must be between 0 and 10` (where `<field>` is `scent`/`flavor`/`power`/`quality`)
